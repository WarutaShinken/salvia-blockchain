...
Write-Output "   ---"
Write-Output "Get SALVIA_INSTALLER_VERSION"
# The environment variable SALVIA_INSTALLER_VERSION needs to be defined
$env:SALVIA_INSTALLER_VERSION = python .\build_scripts\installer-version.py -win

if (-not (Test-Path env:SALVIA_INSTALLER_VERSION)) {
  $env:SALVIA_INSTALLER_VERSION = '0.0.0'
  Write-Output "WARNING: No environment variable SALVIA_INSTALLER_VERSION set. Using 0.0.0"
  }
Write-Output "Salvia Version is: $env:SALVIA_INSTALLER_VERSION"
Write-Output "   ---"
...
Write-Output "   ---"
Write-Output "Build salvia-blockchain wheels"
Write-Output "   ---"
pip wheel --use-pep517 --extra-index-url https://pypi.chia.net/simple/ -f . --wheel-dir=.\build_scripts\win_build .

Write-Output "   ---"
Write-Output "Install salvia-blockchain wheels into venv with pip"
Write-Output "   ---"
...
Write-Output "pip install salvia-blockchain"
...
editbin.exe /STACK:8000000 daemon\salvia.exe
Write-Output "   ---"

$packageVersion = "$env:SALVIA_INSTALLER_VERSION"
$packageName = "Salvia-$packageVersion"
...
If ($env:HAS_SECRET) {
   ...
   Write-Output "   ---"
   signtool.exe timestamp /v /t http://timestamp.comodoca.com/ .\release-builds\windows-installer\SalviaSetup-$packageVersion.exe
   signtool.exe verify /v /pa .\release-builds\windows-installer\SalviaSetup-$packageVersion.exe
   }   Else    {
